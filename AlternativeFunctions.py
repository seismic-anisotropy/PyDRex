...
def strain(pathTime, pathDense, dictGlobals=None):
    ...
    sol = solve_ivp(
        derivIVP, [currTime, pathTime[0]], np.hstack(
            (fse.flatten(), acs0.copy().flatten(), acs0.copy().flatten(),
             np.ones(size) / size, np.ones(size) / size)),
        method='RK45', first_step=dt, max_step=5e12, t_eval=[pathTime[0]],
        events=[eventIVP], atol=1e-8, rtol=1e-5)
