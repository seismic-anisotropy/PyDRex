...
def makeFilledContourPlot(mplAxis, xCoords, yCoords, data, levels, cmap,
                          cbarTicks, cbarLab):
    ctrf = mplAxis.contourf(xCoords, yCoords, data, antialiased=True,
                            levels=levels, cmap=cmap)
    for iso in ctrf.collections:
        iso.set_edgecolor('face')
    mplAxis.set_aspect('equal', adjustable='box')
    mplAxis.set_ylabel('Depth (km)', fontsize=18, fontweight='semibold',
                       labelpad=20, color='xkcd:white',
    ...
    mplAxis.tick_params(which='minor', length=4, width=2,
                        color='xkcd:bright red')
    cbar = fig.colorbar(ctrf, ax=mplAxis, ticks=cbarTicks,
                        fraction=0.05, aspect=10)
    cbar.set_label(cbarLab, labelpad=-25, fontweight='semibold',
                   fontsize=18, color='xkcd:white',
                   path_effects=[path_effects.Stroke(linewidth=3,
                                                     foreground='black'),
                                 path_effects.Normal()])
    cbar.ax.tick_params(which='major', labelsize=16, width=2, length=6)
    cbar.ax.tick_params(which='minor', labelsize=16, width=2, length=3,
                        color='xkcd:bright red')
...
ax.invert_yaxis()
bx.set_xlabel('Y Axis (km)', fontsize=18, fontweight='semibold', labelpad=15,
              color='xkcd:white',
              bbox=dict(boxstyle='round', facecolor='xkcd:black'))
fig.tight_layout()
